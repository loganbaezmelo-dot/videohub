...
const BottomNav = ({ currentUser, currentUserProfile, currentView, onSetView, onNavigateToBytes, onNavigateToChannel }) => {
    // Helper to check if a tab is active for styling
    ...
    return (
        <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex justify-around items-center py-2 z-50 pb-safe">
            {/* Videos (Home) Tab */}
        ...
        <nav className="bg-white dark:bg-gray-900 p-4 text-gray-800 dark:text-white shadow-md sticky top-0 z-40 border-b border-gray-200 dark:border-gray-700">
            <div className="container mx-auto flex items-center justify-between">
                ...
                <div className="flex space-x-2 sm:space-x-4 items-center flex-shrink-0 relative">
                    ...
                    {currentUser ? (
                        <>
                            {/* Upload button hidden on mobile (moved to bottom nav) */}
                            <button onClick={() => onSetView('upload')} className="hidden sm:block px-3 py-2 sm:px-4 sm:py-2 bg-green-600 text-white rounded-full shadow-md hover:bg-green-700 transition-colors text-sm sm:text-base">Upload</button>
                            <div ref={profileMenuRef}>
                                <img 
                                    ...
                                    className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover cursor-pointer border-2 border-transparent hover:border-indigo-500 transition-colors"
...
const ContentGrid = ({ items, onWatch, onNavigateToChannel, title }) => (
    <div className="p-4 sm:p-8 pb-20 sm:pb-8"> {/* Added pb-20 for mobile bottom bar */}
