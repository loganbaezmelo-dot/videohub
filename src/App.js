...
const YahooIcon = () => (
    <svg className="h-5 w-5 mr-2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M2 5L8.5 15V21H11.5V15L18 5H14.5L10 12L5.5 5H2Z" fill="#6001D2"/>
    </svg>
);

const GitHubIcon = () => (
    <svg className="h-5 w-5 mr-2 dark:text-white text-gray-900" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
const BottomNav = ({ currentUser, currentUserProfile, currentView, onSetView, onNavigateToBytes, onNavigateToChannel, showMessage }) => {
    ...
    const handleProtectedAction = (action, viewName) => {
        if (currentUser) {
            action();
        } else {
    ...
    return (
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex justify-between items-center px-2 py-1 z-50 pb-safe">
            ...
            {/* 3. The BIG + Upload Button (Centered & Secured) */}
            <div className="flex-1 flex justify-center relative -top-3">
                <button onClick={() => handleProtectedAction(() => onSetView('upload'), 'upload')} className="bg-indigo-600 dark:bg-indigo-500 text-white rounded-full w-12 h-12 flex items-center justify-center shadow-lg border-4 border-white dark:border-gray-900 active:scale-95 transition-transform">
            ...
            {/* 5. Me (Channel) - Secured */}
            <button onClick={() => handleProtectedAction(() => onNavigateToChannel(currentUser.uid), 'channel')} className={`flex-1 flex flex-col items-center py-2 space-y-1 ${isActive('channel') ? activeClass : inactiveClass}`}>
