...
const SubscriberCount = ({ uploaderId }) => {
    ...
    return <span className="text-sm text-gray-500 dark:text-gray-400">{count} Subscribers</span>;
};
            ...
            <div className="container mx-auto flex items-center justify-between">
                ...
                <div className="flex-grow flex justify-center mx-2 sm:mx-4">
                    <input 
                        ...
                        className="w-full max-w-md px-4 py-2 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 border border-transparent focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    />
                </div>
                
                <div className="flex space-x-2 sm:space-x-4 items-center flex-shrink-0 relative">
                    <button onClick={() => { onSetView('home'); onSearchChange(''); }} className="hidden sm:block px-4 py-2 bg-indigo-600 text-white rounded-full shadow-md hover:bg-indigo-700 transition-colors">Home</button>
                    <button onClick={onNavigateToBytes} className="hidden sm:block px-4 py-2 bg-purple-600 text-white rounded-full shadow-md hover:bg-purple-700 transition-colors">Bytes</button>
                    
                    {currentUser ? (
                        <>
                            <button onClick={() => onSetView('upload')} className="px-3 py-2 sm:px-4 sm:py-2 bg-green-600 text-white rounded-full shadow-md hover:bg-green-700 transition-colors text-sm sm:text-base">Upload</button>
                            
                            <div ref={profileMenuRef}>
                                <img 
                                    src={currentUserProfile?.profilePictureUrl || `https://placehold.co/40x40/7c3aed/ffffff?text=${currentUserProfile?.name ? currentUserProfile.name[0].toUpperCase() : '?'}`}
                                    alt="My Profile"
                                    onClick={() => setIsProfileMenuOpen(!isProfileMenuOpen)}
                                    className="w-10 h-10 rounded-full object-cover cursor-pointer border-2 border-transparent hover:border-indigo-500 transition-colors"
                                    onError={(e) => e.target.src=`https://placehold.co/40x40/7c3aed/ffffff?text=${currentUserProfile?.name ? currentUserProfile.name[0].toUpperCase() : '?'}`}
                                />
                                ...
                                {isProfileMenuOpen && (
                                    <div className="absolute right-0 top-12 mt-2 w-48 bg-white dark:bg-gray-800 rounded-md shadow-lg py-1 border border-gray-200 dark:border-gray-700 z-50">
                                        <button 
                                            onClick={() => { onNavigateToChannel(currentUser.uid); setIsProfileMenuOpen(false); }}
                        ...
                        <button onClick={() => onSetView('landing')} className="px-4 py-2 bg-blue-600 text-white rounded-full shadow-md hover:bg-blue-700 transition-colors">Login</button>
                    )}
                </div>
            </div>
        </nav>
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 flex flex-col">
        <div className="relative cursor-pointer" onClick={() => onWatch(item)}>
            <img 
                ...
                onError={(e) => e.target.src='https://placehold.co/600x400/1f2937/7c3aed?text=Image+Error'}
            />
        </div>
...
const UploadForm = ({ onUpload, showMessage, defaultType }) => {
    ...
    return (
        <div className="max-w-2xl mx-auto p-6 sm:p-8 bg-white dark:bg-gray-800 rounded-lg mt-10 shadow-xl">
...
const EditProfileModal = ({ userProfile, onSave, onCancel }) => {
    ...
    const handleSave = async () => { 
        setIsSaving(true); 
        await onSave({ name, description, profileImageFile }); 
        setIsSaving(false); 
        onCancel(); 
    };
...
function App() {
    ...
    const handleGuestLogin = async () => { 
        try { await signInAnonymously(auth); setView('home'); } 
        catch (e) { showMessageHandler("Could not sign in as guest.", 'error'); }
    };
    const handleSignup = async (email, password) => { 
        try { await createUserWithEmailAndPassword(auth, email, password); showMessageHandler('Account created!', 'success'); setView('home'); } 
        catch (e) { showMessageHandler(e.message, 'error'); }
    };
    const handleLogin = async (email, password) => { 
        try { await signInWithEmailAndPassword(auth, email, password); showMessageHandler('Logged in!', 'success'); setView('home'); } 
        catch (e) { showMessageHandler(e.message, 'error'); }
    };
    const handleGoogleLogin = async () => {
        ...
        try {
            ...
            setView('home');
            showMessageHandler('Logged in with Google!', 'success');
        } catch (error) {
            console.error(error);
    ...
    const handleLogout = async () => { 
        try { await signOut(auth); setView('landing'); showMessageHandler('Logged out.', 'success'); } 
    ...
    const handleWatchContent = (item) => {
        if (item.type === 'byte') { 
            ...
            setBytesPlayerData({ items: allBytes, index: itemIndex >= 0 ? itemIndex : 0 }); 
            setView('bytesPlayer'); 
        } else { 
    ...
    const handleNavigateToBytes = () => { 
        if (allBytes.length > 0) { 
            setBytesPlayerData({ items: allBytes, index: 0 }); 
            setView('bytesPlayer'); 
        } else { 
            setIsNoBytesModalOpen(true); 
        }
    };
    const handleGoToUploadFromModal = () => { 
        setIsNoBytesModalOpen(false); 
        setDefaultUploadType('byte'); 
        setView('upload'); 
    };
    const handleSetView = (view) => { 
        if (view !== 'upload') { setDefaultUploadType('video'); } 
        setView(view); 
        setSearchTerm('');
    };
    ...
    const handleUpload = async ({ videoFile, thumbnailFile, title, description, type, onProgress }) => {
        ...
        const uploadFile = (file, path, cb) => new Promise((res, rej) => { 
            const task = uploadBytesResumable(ref(storage, path), file); 
            task.on('state_changed', 
                (s) => cb && cb((s.bytesTransferred / s.totalBytes) * 100),
                rej,
        ...
        try { 
            ...
            await addDoc(collection(db, `/artifacts/${appId}/public/data/${type}s`), { 
                ...
                uploaderName: currentUserProfile.name || 'User', 
                createdAt: serverTimestamp() 
            }); 
    ...
    const handleSubscribe = async (uploaderId) => {
        ...
        if (snap.empty) { 
            ...
            showMessageHandler('Already subscribed.', 'info'); 
        }
    };
    ...
    const handleUpdateProfile = async ({ name, description, profileImageFile }) => {
        ...
        if (profileImageFile) {
            ...
            catch (error) { showMessageHandler('Failed to upload profile picture.', 'error'); return; }
        }
    ...
    const renderContent = () => {
        if (!isAuthReady) {
            return <div className="flex justify-center items-center h-screen text-xl text-gray-500">Connecting to VideoHub...</div>;
        }
        switch (view) {
        ...
        <div className="bg-gray-100 dark:bg-gray-950 min-h-screen text-gray-900 dark:text-white font-sans">
            ...
            {view !== 'landing' && view !== 'authForm' && (
                <Navbar 
                    ...
                    onNavigateToBytes={handleNavigateToBytes} 
