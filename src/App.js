...
const BottomNav = ({ currentUser, currentUserProfile, currentView, onSetView, onNavigateToBytes, onNavigateToChannel }) => {
    ...
    return (
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex justify-between items-center px-2 py-1 z-50 pb-safe">
            {/* 1. Home */}
            ...
            {/* 3. The BIG + Upload Button (Centered) */}
            <div className="flex-1 flex justify-center relative -top-3">
                <button onClick={() => onSetView('upload')} className="bg-indigo-600 dark:bg-indigo-500 text-white rounded-full w-12 h-12 flex items-center justify-center shadow-lg border-4 border-white dark:border-gray-900 active:scale-95 transition-transform">
            ...
            {/* 5. Me (Channel) */}
            <button onClick={() => currentUser ? onNavigateToChannel(currentUser.uid) : onSetView('landing')} className={`flex-1 flex flex-col items-center py-2 space-y-1 ${isActive('channel') ? activeClass : inactiveClass}`}>
            ...
            <div className="w-full max-w-md p-8 space-y-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg mx-4 my-8">
                ...
                <div className="mt-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button onClick={() => onProviderLogin('google')} className="flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none">Google</button>
                        <button onClick={() => onProviderLogin('github')} className="flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-gray-800 hover:bg-gray-900 focus:outline-none">GitHub</button>
                        <button onClick={() => onProviderLogin('yahoo')} className="flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-purple-600 hover:bg-purple-700 focus:outline-none">Yahoo</button>
                    </div>
                </div>
            </div>
        </div>
...
function App() {
    ...
    useEffect(() => {
        ...
        const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
            if (user && !user.isAnonymous) {
                ...
                const subsQuery = query(collection(db, `/artifacts/${appId}/public/data/subscribers`), where("subscriberId", "==", user.uid));
    ...
    if (view === 'subscriptions') {
        ...
        emptyMsg = "Subscribe to channels to see their videos here!";
        // Filter ALL content to only include subscribed uploaders
        const subVideos = allVideos.filter(v => mySubscriptions.includes(v.uploaderId));
        const subBytes = allBytes.filter(b => mySubscriptions.includes(b.uploaderId)).map(b => ({...b, type: 'byte'}));
        contentToDisplay = [...subVideos.map(v => ({...v, type: 'video'})), ...subBytes].sort((a,b) => (b.createdAt?.toMillis() || 0) - (a.createdAt?.toMillis() || 0));
    } else {
        // Home View (Search filtering applies here)
        contentToDisplay = allVideos.filter(v => 
