...
const firebaseConfig = {
  ...
  storageBucket: "video-website-64dab.firebasestorage.app", // Correct bucket
...
// --- AUTH FORM (Google, GitHub, Yahoo ONLY) ---
            ...
            <div className="w-full max-w-md p-8 space-y-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg mx-4 my-8">
                ...
                <div className="mt-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        {/* YAHOO - Full width below */}
                        <button onClick={() => onProviderLogin('yahoo')} className="col-span-2 flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-purple-600 hover:bg-purple-700 focus:outline-none">
...
// --- SETTINGS MODAL with Account Linking ---
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
                ...
                <div className="flex border-b border-gray-200 dark:border-gray-700">
                    ...
                    <button onClick={() => setActiveTab('security')} className={`flex-1 py-3 text-sm font-medium text-center ${activeTab === 'security' ? 'text-indigo-600 border-b-2 border-indigo-600 dark:text-indigo-400 dark:border-indigo-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}>Security</button>
                </div>
...
function App() {
    ...
    const handleLogout = async () => { 
        try { 
            if (currentUser && localStorage.getItem('sessionId')) {
                await deleteDoc(doc(db, `/artifacts/${appId}/public/data/users/${currentUser.uid}/sessions`, localStorage.getItem('sessionId')));
            }
            localStorage.removeItem('sessionId');
            await signOut(auth); 
            setView('landing'); 
    ...
    // --- UPDATED HANDLE UPLOAD with Smart Thumbnails ---
    const handleUpload = async ({ videoFile, thumbnailFile, title, description, type, onProgress }) => {
        ...
        try { 
            ...
            if (thumbnailFile) {
                ...
                // Use Placehold.co API for placeholder thumbnails
