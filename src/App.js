...
const UploadForm = ({ onUpload, showMessage, defaultType }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        // Description is optional, so we just get the value
        ...
        if (!videoFile || !title) { showMessage({ text: 'Please select a video and enter a title.', type: 'error' }); return; }
    ...
    return (
        <div className="max-w-2xl mx-auto p-6 sm:p-8 bg-white dark:bg-gray-800 rounded-lg mt-10 shadow-xl mb-20 sm:mb-0">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <select value={fileType} onChange={(e) => setFileType(e.target.value)} className="w-full px-3 py-2 text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <option value="video">Video (Long Form)</option>
                    <option value="byte">Byte (Short Form)</option>
                </select>
                ...
                <div>
                    ...
                    <input type="file" id="video-file" accept="video/*" required className="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 mt-1" />
                </div>

                {/* Thumbnail Input - Only show for VIDEOS */}
                {fileType === 'video' && (
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Thumbnail Image (Optional)</label>
                        <input type="file" id="thumbnail-file" accept="image/*" className="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100 mt-1" />
                    </div>
                )}
                ...
                <div className="p-6 overflow-y-auto flex-1">
                    ...
                    {activeTab === 'security' && (
                        <div>
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">These are the devices currently logged into your account. Remove a session to log it out.</p>
        ...
        try { 
            ...
            if (thumbnailFile) {
                ...
                thumbnailUrl = type === 'byte' 
                    ? `https://placehold.co/400x600/7c3aed/ffffff?text=Byte` 
                    : `https://placehold.co/600x400/4338ca/ffffff?text=Video`;
            }
