...
// --- NEW: Enhanced Settings Modal with Devices ---
...
function App() {
    ...
    const checkSessionValidity = async (user) => {
        ...
        if (!sessionId) return true; // No session ID stored, allow login to proceed (will generate one)
    ...
    const handleRevokeSession = async (sessionId) => {
        ...
        try {
            ...
            // If we revoked our own session, log out locally
    ...
    useEffect(() => {
        ...
        const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
            if (user && !user.isAnonymous) {
                ...
                // Sessions Listener (for Settings)
    ...
    const handleGoogleLogin = async () => { 
        ...
        try { 
            // Set persistence to local (keeps you logged in even after refresh)
    ...
    const handleLogout = async () => { 
        try { 
            // Optional: Remove current session from DB on logout
